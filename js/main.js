...
async function concatenateFiles(filePaths) {
  try {
    ...
    fs.writeFile('./dist/bundle.js', concatenatedContent, 'utf8', (error) => {
      if (error) {
        //console.error('Error writing file:', error);
      } else {
      //  console.log('Files concatenated successfully!');
			const originalCode = fs.readFileSync('./dist/bundle.js', 'utf8');

			const minifiedCode = uglifyjs.minify(originalCode, { compress: true, mangle: true });

			fs.writeFileSync('./dist/bundle.js', minifiedCode.code, 'utf8');
